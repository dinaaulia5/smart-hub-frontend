...
const fetchBooking = async () => {
    const response = await bookingService.getById(props.id);
    console.log(response.data.data);

    const booking = response.data.data;

    form.value.start_time = formatDateTimeLocal(booking.start_time);
    form.value.end_time = formatDateTimeLocal(booking.end_time);

    form.value.items = booking.items.map((item) => ({
        bookable_type: item.bookable_type.toLowerCase(),
...
const onHandleSubmit = async () => {
    ...
    try {
        ...
        await bookingService.update(props.id, form.value);
